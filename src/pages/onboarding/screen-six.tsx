...
export default function ScreenSix({ isMobile }: { isMobile: boolean }) {
  return (
    <div
      className={cn(
    ...
    >
      {isMobile && <div className="z-10 w-full h-[50%] bg-transparent"></div>}
      <div
        className={cn(
          "w-full flex flex-col items-center gap-y-0",
          isMobile
            ? "h-[50%] z-10 justify-end bg-transparent"
            : "justify-center max-w-md mx-auto w-full"
        )}
      >
        <div
          className={
            "size-full flex flex-col items-center justify-center text-center px-4 h-fit py-4 gap-y-4"
          }
        >
          <div className="size-full flex justify-center items-center h-[120px] flex-col gap-y-3">
            {isMobile ? (
              <img
            ...
            <p
              className={cn(
                "leading-relaxed max-w-md font-light text-base h-[fit]",
                isMobile && "text-white"
              )}
            >
        ...
        <div
          className={cn(
            "size-full flex items-center justify-center px-6 gap-4 gap-x-1 py-6 h-[100px]",
            isMobile && "text-white"
          )}
        >
          <p
            className={"leading-relaxed max-w-md text-base h-[fit] font-normal"}
          >
            Already have an account? 
          </p>
          <Link
            to={"/login"}
            className={"leading-relaxed max-w-md text-base h-[fit] font-medium"}
          >
            Sign in
          </Link>
        </div>
