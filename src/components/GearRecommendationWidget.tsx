...
const GearRecommendationWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleLocationChange = (event: CustomEvent) => {
      console.log(
        "[GearRecommendation] Received location change event:",
        event.detail,
      );
      loadLocation();
    };
    ...
    const startLocationPolling = () => {
      locationCheckInterval = setInterval(() => {
        ...
        if (currentLocationString !== lastLocationString) {
          console.log(
            "[GearRecommendation] Location change detected via polling",
          );
  ...
  useEffect(() => {
    if (!location) {
      console.log(
        "[GearRecommendation] No location available, skipping analysis",
      );
      return;
    }

    if (userGear.length === 0) {
      console.log("[GearRecommendation] No gear found, skipping analysis");
      return;
    }

    console.log("[GearRecommendation] Prerequisites met:", {
    ...
    if (hasLocationChanged) {
      console.log("[GearRecommendation] Location changed, resetting analysis");
      setAnalysis({
        phase: "idle",
        weatherConditions: null,
        recommendations: [],
    ...
    if (analysis.phase === "idle" || hasLocationChanged) {
      console.log("[GearRecommendation] Starting analysis sequence");
  ...
  const fetchWeatherData = async (): Promise<WeatherConditions> => {
    ...
    console.log(`[GearRecommendation] Fetching weather for: ${location.name}`);
  ...
  const generateAIRecommendations = async (
    ...
    if (!import.meta.env.VITE_OPENAI_API_KEY) {
      console.log("[GearRecommendation] No OpenAI API key available");
      return [];
    }
