...
export interface FAOToxicFishStreamEvent {
  type: "init" | "status" | "toxic_fish" | "progress" | "complete" | "error";
  message?: string;
  data?: any;
  count?: number;
  checked?: number;
...
export interface UseFAOToxicFishStreamReturn {
  // Fish data
  toxicFish: FishData[];
  allFish: FishData[];
...
export interface UseFAOToxicFishStreamOptions {
  latitude?: number;
  longitude?: number;
  autoStart?: boolean;
}

export function useFAOToxicFishStream(
  options?: UseFAOToxicFishStreamOptions,
): UseFAOToxicFishStreamReturn {
  const { latitude, longitude, autoStart = false } = options || {};

  const [toxicFish, setToxicFish] = useState<FishData[]>([]);
  ...
  const handleEvent = useCallback((event: FAOToxicFishStreamEvent) => {
    switch (event.type) {
      case "init":
      ...
      case "toxic_fish":
        if (event.data) {
          ...
          if (isNewFish(transformedFish.scientificName, seenFishRef)) {
            setToxicFish((prev) => [...prev, transformedFish]);
          }
        }
        break;
  ...
  const handleStreamChunk = useCallback(
    (chunk: string) => {
      parseStreamChunk<FAOToxicFishStreamEvent>(
        chunk,
        streamBufferRef,
        handleEvent,
      );
    ...
    async (overrideOptions?: { latitude?: number; longitude?: number }) => {
      ...
      try {
        ...
        const stream = await apiStreamed(
          `fao/fish/toxic/stream?${queryParams.toString()}`,
          {
            method: "GET",
  ...
  const allFish = useMemo(() => toxicFish, [toxicFish]);
